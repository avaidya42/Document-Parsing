...
def generate_llm_prompt(unstructured_text: str):
    ...
    Natal. These may be labeled as "Lasik" or "Lasik Surgery". If the document specifies correction index, extract the number as eye_power.
    Normalize formatting errors in the number and treat them equivalently.
    If no specific eye power is mentioned or the number is ambiguous, return 0.0.
    Also extract si_limit if mentioned. If not mentioned, return "".
    ...
    17. If the document says "9 months waiting period: Waive off" or similar (e.g., "9 month waiting period waived off for all insured members"), then map the maternity waiting_period as "Waived off" (instead of "9 Months").

    18. If the policy says "All Day Care Procedures are covered" or "covered as per IRDA list" or something similar , treat that as day_care_treatment = "Covered".

    19. If the policy says "Donor expenses are not covered", set organ_donor_expenses = "Not Covered".
    
    20. If "Modern Treatment" or "Advanced Technology" is covered up to a percentage (e.g. 50%), set medical_advancement_surgery_limit accordingly.

    21. Only extract 'total_sum_insured' if it is clearly and explicitly stated in the document. Do not infer it by summing individual values or reading into benefit tables.
